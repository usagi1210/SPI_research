...
# Args
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser(description='Test ISTA-Net+')
parser.add_argument('--cs_ratio',   type=int,   default=25,    choices=[1, 4, 10, 25, 30, 40, 50])
parser.add_argument('--num_layers', type=int,   default=9)
parser.add_argument('--epoch_num',  type=int,   default=200,   help='which checkpoint to load')
parser.add_argument('--use_best',   action='store_true',        help='load best_model.pth instead of epoch checkpoint')
parser.add_argument('--gpu',        type=str,   default='0')
parser.add_argument('--test_set',   type=str,   default='Set11', help='folder name under data/test/')
...
parser.add_argument('--result_dir', type=str,   default='../../results/ISTA_Net/images')
...
# Load model
# ---------------------------------------------------------------------------
ckpt_prefix = f'ista_net_plus_layer{args.num_layers}_ratio{args.cs_ratio}'
if args.use_best:
...
print(f'\nTesting on {args.test_set} | ratio={args.cs_ratio}% | {len(img_paths)} images\n')

with torch.no_grad():
    for idx, img_path in enumerate(img_paths):
        img_bgr = cv2.imread(img_path, cv2.IMREAD_COLOR)
        img_yuv = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2YCrCb)
        Iorg_y  = img_yuv[:, :, 0]                   # Y channel

        Iorg, row, col, Ipad, row_new, col_new = imread_cs(Iorg_y)
        Icol = img2col(Ipad) / 255.0                  # (N_blocks, 1089)
        ...
        pred = x_out.cpu().numpy()                    # (N_blocks, 1089)
        ...
        # Save reconstructed image (full colour)
        img_rec_yuv       = img_yuv.copy()
        img_rec_yuv[:, :, 0] = (X_rec * 255).astype(np.uint8)
        img_rec_bgr       = cv2.cvtColor(img_rec_yuv, cv2.COLOR_YCrCb2BGR)
        base              = os.path.splitext(os.path.basename(img_path))[0]
        save_path         = os.path.join(save_dir, f'{base}_PSNR{psnr_val:.2f}_SSIM{ssim_val:.4f}.png')
        cv2.imwrite(save_path, img_rec_bgr)
...
summary  = (f'\nAvg PSNR={avg_psnr:.2f}  Avg SSIM={avg_ssim:.4f} '
            f'| set={args.test_set}  ratio={args.cs_ratio}%  '
            f'epoch={args.epoch_num}\n')
print(summary)
